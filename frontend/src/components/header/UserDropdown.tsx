    ...
    <div className="relative">
      ...
      >
        <span className="mr-3 flex h-11 w-11 items-center justify-center overflow-hidden rounded-full bg-brand-500 text-sm font-semibold text-white">
          {meta.avatar ? (
            <Image width={44} height={44} src={meta.avatar} alt={meta.name} className="h-11 w-11 object-cover" />
          ) : (
            initials(meta.name)
          )}
        </span>

        <span className="block mr-1 font-medium text-theme-sm">{meta.name}</span>
