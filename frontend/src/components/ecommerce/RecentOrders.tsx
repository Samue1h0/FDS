...
function getStatusInfo(txn: Transaction): {
  ...
  if (txn.reviewed_by && txn.reviewed_by !== "") {
    return { label: "Resolved", color: "success" };
  }
  if (txn.predicted_label === "FRAUD") {
...
export default function RecentTransactions({ transactions }: RecentOrdersProps) {
  return (
              ...
              transactions.map((txn) => {
                ...
                return (
                  <TableRow key={txn.transaction_id}>
                    ...
                    <TableCell className="py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                      <Badge size="sm" color={risk.color}>
                        {risk.label}
                      </Badge>
                    </TableCell>
                    <TableCell className="py-3">
